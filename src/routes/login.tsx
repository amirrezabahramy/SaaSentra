...
function LoginPage() {
  ...
  const form = useForm({
    ...
    validators: {
      onSubmit: ({ value }) =>
        !value.email || !value.password ? 'Email and password are required' : undefined,
    },
  })

  return (
    <main className="min-h-screen grid place-items-center p-8">
      <form onSubmit={(event) => { event.preventDefault(); void form.handleSubmit() }} className="w-full max-w-sm space-y-4">
        <div>
        ...
        <form.Field name="email">
          {(field) => <label className="block space-y-1">
        ...
        {signInMutation.isError ? <p className="text-sm text-red-600">Invalid email or password</p> : null}
